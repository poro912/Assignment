...
- `struct sockaddr *restrict address`
  ...
  - sockadr_* 구조체이며 sockaddr 구조체로 캐스팅 해야 함
  - 정보가 필요 없다면 NULL로 지정
- `socklen_t *restrict address_len`	: sockaddr 구조체의 크기 (정보가 필요 없다면 NULL)
...
accept가 계속해서 루프를 돌고있다면 매번 sockaddr 매개변수를 세팅해서 넣어야 한다.
...
- `struct socaddr *address`	: sockadr_* 구조체이며 sockaddr 구조체로 캐스팅 해야 함
...
**ErrorCode**
| EAGIN | 넌블로킹으로 세팅된 소켓에서 소켓 버퍼에 공간이 없는 경우</br>보통 재전송시도를 한다.|
| :--: | :--: |
| EWOULDBLOCK	| 넌블로킹으로 세팅된 소켓에서 소켓 버퍼에 공간이 없는 경우</br>보통 재전송시도를 한다. |
| EINTER	| 인터럽트가 발생하여 전송이 중단되었다.</br>보통 재전송시도를 한다. |
| EPIPE		| 반대편 소켓의 연결이 끊어졌다.</br>보통 연결을 닫고 정리한다. |
| ECONNRESET	| 반대편에서 연결을 강제로 끊었다.</br>보통 연결을 닫고 정리한다. |
...
- `int flags` 
  - 작동 플래그
	...
	| MSG_WAITALL	| 버퍼 크기가 다 채워질 때까지 대기한다.</br>시그널 개입이나 연결이 끊어진 경우엔 에러로 리턴된다. |
