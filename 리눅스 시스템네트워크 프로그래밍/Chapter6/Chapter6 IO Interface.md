...
- [프로토콜 헤더](#프로토콜-헤더)
	...
	- [UDP 헤더](#udp-헤더)
- [TCP 기법](#tcp-기법)
	- [네이글 알고리즘 (지연 ACK)](#네이글-알고리즘-지연-ack)
	- [슬라이딩 윈도우](#슬라이딩-윈도우)
	- [TCP autotuning](#tcp-autotuning)
- [UDP 기법](#udp-기법)
	- [UDP 브로드 캐스팅](#udp-브로드-캐스팅)
- [IPv6 소켓 프로그래밍](#ipv6-소켓-프로그래밍)
- [소켓 옵션](#소켓-옵션)
...
## TCP 기법
### 네이글 알고리즘 (지연 ACK)
너무 작은 데이터그램은 네트워크 구간에서 혼잡을 일으킨다.  
이를 해결하기 위해 짧은 시간동안 작은 패킷을 모아 ACK가 수신되면 전송하도록 한다.  
200msec 정도의 작은 시간동안 작은 패킷을 모아두었다가 ACK가 수신되면 전송하도록 하는 알고리즘이다.  
수신측에서 응답이 필요할 때 패킷을 포장하며 ACK 번호를 실어보내 ACK 패킷을 따로보내지 않는다.  
흐름제어 역할을 한다.  

ACK(긍정 응답 패킷) : TCP 프로토콜에서 수신측이 패킷을 받았음을 알리는 응답 패킷  
피기백 : 데이터가 생길 때까지 잠시 기다렸다 ACK 번호를 함께 보낸다.  
...
2. 수신측의 읽기요청으로인해 버퍼가 비워질 때
   - recv 함수를 사용하는 경우
   - 슬라이딩 윈도우의 절반 이상의 데이터를 읽은 경우, MSS 크기의 두배 이상의 데이터를 읽은 경우
   - 많은 데이터를 읽은 경우 슬라이딩 윈도우 크기를 조정하기위한 응답 패킷을 보낸다.
   - 이 때 ACK 필드를 채워 보내게 된다.

setcokopt를 통해 소켓 옵션을 TCP_NODELAY로 설정하면 네이글 알고리즘을 사용하지 않을 수 있다.  
...
송신측은 수신측 버퍼의 크기를 알고 그 값만큼 응답이 없어도 보낼 수 있도록 하여 전송속도를 높인다.  
...
| net.ipv4.tcp_moderate_rcvbuf            | 수신 버퍼에 TCP autotuning을 적용한다.</br>0 : disable</br>1 : enable |
| :-------------------------------------- | :-------------------------------------------------------------------- |
...
setsockopt를 이용해 값을 변경할 수 있다.  
만약 버퍼 크기를 rmem_max와 wmem_max 보다 큰 값으로 설정하면 무시된다.  

과거 최대 속도를 간주하고 소켓 버퍼를 큰 크기로 고정하여 프로그래밍 할 때  
대용량 소켓 버퍼를 사용하는 프로세스가 많은 소켓을 생성하면 메모리가 비정상적으로 할당돼 문제가 생긴다.  
최대 접속 개수, 메모리 부족으로 인하여 타 프로세스의 성능에도 영향을 준다.  

해결 방법에는 두 가지가 있다.  
소켓을 많이 생성하는 프로세스는 같은 호스트에서 구동하지 않도록 운영하는 방법  
소스 코드를 직접 수정할 수 있는 경우라면 TCP autotuning이 적용된 시스템에서  
SO_RCVBUF, SO_SNDBUF를 설정을 on off 할 수 있도록 수정하는 방법  


## UDP 기법
### UDP 브로드 캐스팅
브로드 캐스팅 : 1 : n 대응으로 데이터를 보낼 수 있는 기능  
IPv4에서만 지원되며 IPv6에서는 멀티캐스트가 대신한다.  
...
- `int option_name` : 변경할 옵션 대상
- `void *option_value` : 변경할 값 (경우에 따라 구조체를 사용한다.)
- `socklen_t *option_len` : 변경할 값의 길이
...
TCP 클라이언트
- connect 이전에 특정 주소로 explicit binding을 하는 경우
...
### SO_RCVBUF, SO_SNDBUF
모든 타입의 소켓에 사용가능하다.  
소켓 버퍼는 socket 함수가 성공하면 생성된다.  
수신 버퍼와 송신버퍼로 이루어져있다.  
소켓 버퍼의 크기는 전송 속도나 의미있는 데이터 분절의 최대 크기에 영향을 준다.  
전송 속도나 안정성 면에서는 큰 편이 좋다.  	
큰 소켓 버퍼는 성능을 성능을 갉아먹는 주범이 될 수 있다.  
큰 소켓 버퍼를 사용하면 소켓의 개수가 늘어날수록 기용할 수 있는 메모리는 빠르게 고갈된다.  
빈번하게 소켓을 생성하고 파괴하는 구조라면 소켓의 할당과 해제에 오버헤드가 발생한다.  
시스템에 손상을 가할 정도의 메모리(메모리의 70% ~ 85%)가 네트워킹에 할당되면 기존 소켓의 메모리를 줄이거나 새로운 소켓 생성을 거부한다.  

TCP autotuning 같은 기능을 이용해 동적 크기의 버퍼를 가질 수 있다.  
이를 이용해 기존에 생성된 소켓의 버퍼크리를 줄여 시스템이 가용할 수 있는 메모리를 확보한다.
소켓 버퍼의 크기가 줄어드는 만큼 전송속도가 줄어든다는 단점이 생긴다.
시스템에 소켓의 개수가 늘어나면서 전송 속도가 줄어든다면 메모리의 확충이 필요한지 검사를 해야한다.

리눅스  
- 소켓 버퍼 크기를 설정 한 후 값을 읽어보면 2배의 수치로 설정되는 경우가 존재한다.  
- 이 경우 소켓 버퍼 구조체가 부가적인 정보를 저장하기 때문이다.  
- 따라서 확보된 크기보다 작은 크기의 버퍼를 사용하게 된다.  
...
TCP
- TCP 에서 소켓 버퍼는 각 데이터 세그먼트들이 저장되는 스트림 메모리 공간이다.  
- 수신 버퍼의 경우 윈도우 크기와 비례하거나 동일하다.  
- 전송속도를 높이기 위해 수신 버퍼의 크기를 늘리는 경우도 많다.  
- 상대적으로 송신 버퍼는 수신버퍼의 완충 역할이므로 작아도 문제는 생기지 않는다.  
- 송신측의 송신 버퍼와 수신측의 수신버퍼는 서로 연결된 스트림처럼 인식하기 때문이다.  
- 대용량 소켓 버퍼보다 데이터를 수신하는 부분을 더 고성능의 구조로 설계하고 만드는 것이 중요하다.  
- 데이터를 수신하는 부분은 최소한의 작업만 하여 가볍게 만들거나 비동기적으로 만들어 둔다.  
- 수신 후 데이터를 가공하는 경우, 스레드에서 처리하거나 쌓아둔 후 조금 뒤에 처리하는 방식을 사용하기도 한다.  
- 로컬 네트워크의 경우 대부분 지연이 없고 대역폭도 넓어 소켓 버퍼가 작아도 문제가 발생하지 않는다.  
- 수동으로 작은 버퍼를 사용하도록 하여 메모리를 적게 사용하는 구조로 작성하기도 한다.  


UDP
- UDP에서 수신 버퍼는 단편화된 조각들을 모아 데이터그램을 완성하여 넣어주는 곳이다.  
- 완성된 데이터그램이 들어갈 수 있는 충분한 크기가 요구된다.  
- 데이터 그램의 크기보다 수신측 버퍼의 크기가 작다면 저장할 수 없으므로 버려진다.  
- 따라서 데이터 그램의 최대 크기를 작게하고 빈도수를 약속해놔야 한다.  
- 송신 버퍼가 데이터그램보다 작은 경우 전송은 가능하다.
- 데이터그램이 프래그먼테이션 처리되면서 전송되기를 기다려야하기 때문에 응답속도가 떨어진다.


TCP autotuning 기능을 적극적으로 사용하는 것이 좋다
만일 소켓 버퍼 크기를 수동으로 관리하면 성능은 오히려 떨어지고 메모리를 과도하게 사용할 가능성이 커진다.
과거에 수동설정으로 개발된 네트워크 프로그램이라면 SO_RCVBUF 기능을 비활성화 하는 것이 더 좋은 경우가 많다.


### SO_RCVTIMEO, SO_SNDTIMEO
TCP, UDP 모두에서 사용할 수 있다.  
struct timeval 구조체를 인수로 사용한다.  
send, sendto, connect 와 같이 송신을 목적으로 하는 함수가 블록되었을 대 타임아웃 시간을 지정한다.  
타임아웃 시간이 지나도록 함수가 리턴되지 않는다면 함수는 에러로 리턴되고 errno는 EAGAIN으로 설정된다.


### SO_LINGER
TCP에서 사용가능하다.  
연결을 끊을 때 송신 버퍼에 데이터가 완전히 전송되었는지 확인하기 위해서 사용된다.  
...
연결을 끊고자 할 때 데이터를 보내기 위해 대기하는 시간을 의미한다.  
close, shutdown이 실행될 대 블록될 타임아웃을 의미한다.  
...
l_linger가 0인 경우 대기하지 않겠다는의미이다.  
연결을 끊을 때 즉시 버퍼를 파괴하며, 송신 버퍼에 데이터가 있다해도 버퍼가 즉시 파괴된다.  
따라서 데이터가 상대편 피어(peer)에게 전달되지 않는다.  
이후 연결을 갖에로 취소하기 위해 RST 세그먼트를 보낸다.  
이때문에 active close를 하더라도 TIME_WAIT이 발생하지 않는다.  
...
l_linger가 0이 아닌 경우 연결을 끊을 때 해당 시간만큼 기다린다.  
시간 내에 전송이 완료되었다면 일반적인 연결해제와 같이 작돈되어 TIME_WAIT 상태로 간다.  
...
TCP에서 사용가능하다.  
일정 시간마다 연결 상태를 검사한다.
프로그래머가 아무런 데이터를 전송하지 않아도 확인용 패킷을 보내어 통신 구간이 문제 없는지 확인한다.
기본값 : off

30분 ~ 2시간 사이가 기본값이다. 
동적으로 조절 되는 경우도 존재한다.
리눅스에서는 TCP_KEEOIDLE, TCP_KEEPINTVL 옵션으로 조정할 수 잇다.
...
이런경우 데이터를 보내고자 할 때 에러가 발생하는데 이를 방지하기 위해 사용한다.
...
마우스 움직임을 감지해야 하는 경우, 원격 터미널 또는 LAN구간에서 인터랙티브(상호 활동적)한 데이터를 주고받는 경우에 사용한다.
작은 패킷이 부담을 주는 경우는 WAN구간에 해당한다.
WAN에서도 응답성이 중요한 경우(게임, 채팅 프로그램)에는 사용한다.
