...
| 데이터 크기 제한 	| 없음			| 보존 됨		| 
| 데이터 순서 		| 보존됨		| 보존 안 됨		| 
| 연결 과정 		| 필요함 (1:1 통신)	| 필요 없음 (1:n 통신)	| 
| 신뢰 			| 높음 (데이터 유실시 재전송) | 낮음 (유실 시 복구 X)	| 
...
- `struct sockaddr *addr`
  ...
  - sockaddr_* 형태의 구조체를 사용한다.
  - (struct sockaddr *) 타입으로 캐스팅해 사용한다.
  - 포트 번호를 빅엔디안으로 저장해 사용 한다.
  - htons 함수를 사용해 포트를 지정한다.
...
- `int flags`
  - 작동 플래그
	   ...
	   |      0       | write와 동일하게 동작한다.                                                                                                 |
	   |   MSG_OOB    | 아웃오브밴드 데이터를 송신한다.                                                                                            |
	   | MSG_NOSIGNAL | 반대편 소켓 연결이 끊어졌을 때 SIGPIPE 시그널을 발생시키지 않는다.</br>EPIPE 에러 설정은 여전히 작동한다.                  |
...
|   단편화    | MSS크기로 세그먼테이션하므로 프래그먼테이션은 발생하지 않는다.(MSS < MTU)</br>수신처의 수신 대기큐는 들어오는 세그먼트를 처리하여 재조합을 한다.</br>누락되면 재전송되어 도착할 때까지 재조합은 보류된다. | IP계층에서 단편화가 일어난다.</br>자체적으로 세그먼테이션하지 않는다.</br>조각난 단편 중 하나가 유실되거나 체크섬에 실패한 데이터그램은 삭제된다.</br>재전송은 일어나지 않는다. |
|  흐름제어   | 슬라이딩 윈도우를 사용하여 흐름제어를 보장한다.                                                                                                                                                           | 흐름제어를 하지 않는다.</br>전송이 요청되면 곧바로 송신한다.</br>수신처에서 오버플로우될 수 있다.                                                                            |
|  에러 처리  | 체크섬으로 검증한다.</br>실패시 TCP 프로토콜상에서 재전송한다.                                                                                                                                                 | 체크섬으로 검증한다.</br>실패시 요청은 삭제된다.                                                                                                                                  |
...
Total Length
- 패킷의 길이
- IP 패킷의 길이가 MTU보다 크다면 단편화가 발생한다.
...
- DF와 MF가 있다. 
  ...
  - MF (More Freagmentation)
    - 추가로 단편화된 서브 패킷이 존재한다. (추후 패킷이 더 존재함)
    - Fregment Offset을 사용한다.

Fregment Offset
- 단편화된 상대적인 위치 정보를 가리킨다.
- 이 정보를 통해 단편화된 데이터를 순서에 맞게 재조합할 수 있다.
- 단편화된 마지막 서브 패킷은 플래그에 0이 지정된다.
...
| 커널 sysctl 속성 | 설명 |
| :-------------------------------------- | :-------------------------------------------------------------------- |
| net.ipv4.tcp_moderate_rcvbuf            | 수신 버퍼에 TCP autotuning을 적용한다.</br>0 : disable 1 : enable |
...
과거에는 rmem_max와 wmem_max 최대속도를 간주하고 소켓 버퍼를 큰 크기로 고정하여 프로그래밍 하였다.  
대용량 소켓 버퍼를 사용하는 프로세스가 많은 소켓을 생성하면 메모리가 비정상적으로 할당돼 문제가 생겼다.  
메모리의 비정상적 할당은 클라이언트의 최대 접속 개수, 메모리 부족으로 인하여 타 프로세스의 성능에도 영향을 준다.  
...
IPPROTO_IP, IPPROTO_IPV6 레벨의 옵션도 있지만 일반적으로 잘 쓰이지 않는다.  
man 7 ip, man 7 udp 페이지를 참조하여 프로그램을 작성한다.  
...
- 데이터그램의 크기보다 수신측 버퍼의 크기가 작다면 버려진다.  
