...
- [메시지큐](#메시지큐)
	- [메시지 큐 시스템 크기 확인](#메시지-큐-시스템-크기-확인)
- [XSI IPC](#xsi-ipc)
...
### 메시지 큐 시스템 크기 확인
보통 65535byte 이다.  
메시지 큐의 크기는 시스템 설정마다 다르다.  
설정을 보는 방법은 ipcs 명령으로 확인할 수 있다.  
`ipcs -q`	: 메시지 큐의 최대 크기  
`ipcs -q -l`	: 메시지의 최대 크기  
...
무한 대기 상태가 발생할 수 있으므로 넌블럭킹 모드를 사용하는게 신뢰성을 높일 수 있다.  
post실행 시 OVERFLOW 에러가 발생하면 계속 세마포어를 증가시키는 구간이 있는지 확인해야 한다.  
...
mq_maxmsg와 mq_msgsize는 커널에서 지정한 설정보다 크게 설정하면 에러가 발생한다.  
